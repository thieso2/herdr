...
Menus. Every menu the sidebar opens on a click has a key, so nothing in
Herdr is mouse-only — including adding your first remote, which lives in
the remotes menu on a fresh install with an empty fleet:

| Action | Key |
| --- | --- |
| Global menu (settings, keybinds, add remote, detach) | `prefix+m` |
| Remotes menu (new remote, edit remotes) | `prefix+shift+m` |
| Spaces menu | `prefix+shift+s` |
| Agents menu | `prefix+shift+a` |

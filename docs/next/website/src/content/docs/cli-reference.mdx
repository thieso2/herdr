...
`remote list` reads `remotes.toml` even with no server running. `remote reset` drops one remote's connection and reconnects immediately, which is also how an `incompatible` remote is retried after an upgrade. `remote reload` applies hand edits to `remotes.toml`.

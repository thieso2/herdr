...
```bash
herdr remote list [--json]
herdr remote reset <name> [--json]
herdr remote start <name> [--json]
herdr remote reload [--json]
herdr remote upgrade <name-or-target> | --all [--yes] [--json]
```

`remote start` starts a `stopped` remote's server and connects to it. Connecting never starts one on its own, so this is how a reachable host with Herdr installed but nothing running comes up. `remote list` reads `remotes.toml` even with no server running. `remote reset` drops one remote's connection and reconnects immediately, which is also how an `incompatible` remote is retried after an upgrade. `remote reload` applies hand edits to `remotes.toml`.

`remote upgrade` rolls remotes forward to the version your update channel offers, one host at a time. It accepts a configured remote name or a bare ssh target, never downgrades a host that is already newer, and requires `--yes` when stdin is not interactive. Connecting to a remote never upgrades it; this command is the only thing that does. See [Mixed versions and the upgrade window](/docs/persistence-remote/#mixed-versions-and-the-upgrade-window).
...
Attach and terminal sessions stream the pane's own output: the first
`terminal.frame` record carries `full: true` with a snapshot of the current
screen, and later records carry the raw output that followed it. Writable
attach is exclusive per pane. A second writer without `--takeover` is refused
with a `pane_write_locked` error instead of losing its connection, and
`--takeover` revokes the previous writer, which detaches with

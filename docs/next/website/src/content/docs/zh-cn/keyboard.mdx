...
菜单。侧边栏点击能打开的每个菜单都有对应按键，因此没有任何操作只能靠鼠标完成——包括在远端列表为空的全新安装上添加第一个远端，该操作位于远端菜单中:

| 动作 | 按键 |
| --- | --- |
| 全局菜单 (设置、按键绑定、添加远端、分离) | `prefix+m` |
| 远端菜单 (新建远端、编辑远端) | `prefix+shift+m` |
| 空间菜单 | `prefix+shift+s` |
| 智能体菜单 | `prefix+shift+a` |

...
| 2026-07-27 | A fresh install had no remotes section at all, so there was nowhere to add a first remote | The strip was composed only when it had a chip to show. Removing the implicit `local` row made an unconfigured fleet genuinely empty, so `split_sidebar_for_chip_strip` bailed on `chips.is_empty()` and `sidebar_section_header_rects` gated the header on `has_chips` - reopening, for zero remotes, the bootstrap hole efe90863 closed for one | `src/client_state/run.rs::an_empty_fleet_still_composes_the_remotes_strip` (live: `remotes▾` present with no `remotes.toml`) | this commit |
| 2026-07-27 | `esc` dismissed nothing in the pure client - no dialog, no prefix cancel, no copy-mode exit | `RawInputFramer::push` holds a lone `esc` back because it cannot yet tell it from the first byte of a sequence; only `flush_timeout` resolves that. The pure client owns its own framer but never called it, so the keystroke sat buffered until the *next* one pushed it out. Unit tests inject `TerminalKey` directly and so bypassed the framer entirely | `src/client_state/run.rs::an_idle_tick_releases_a_held_escape_and_closes_a_dialog` (live: keymap overlay survives `esc` on 0.7.108, dismisses after the fix) | 166527b5 |
| 2026-07-27 | Stopping a remote from the remotes dialog left its spaces and pane on screen | The announced-stop path parked the chip (`connection.stopped` + `Link::Stopped`) but never dropped what the client had mirrored, and `drop_link` early-returns on a stopped link so the EOF that follows could not clean up either. The dead remote kept contributing spaces and a replica to the composed view | `src/client_state/run.rs::an_announced_stop_drops_the_remotes_spaces_and_panes_from_the_view` | this commit |
